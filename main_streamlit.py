...
def get_stations():
    ...
    stations_real_time['coordinates'] = stations_real_time['geometry'].apply(lambda x: x['coordinates'])
    stations_real_time[['longitude', 'latitude']] = pd.DataFrame(stations_real_time['coordinates'].tolist(), index=stations_real_time.index)
    stations_real_time = stations_real_time.drop('coordinates', axis=1)
    ...
    st.dataframe(stations_real_time)
